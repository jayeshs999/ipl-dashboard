...
import { RunsChartOptions } from './runs-chart.config';
  ...
  ngAfterViewInit() {
    this.route.params.subscribe((params: any) => {
      ...
      this.http.get(API.serverURL + API.getMatch + this.matchID).subscribe((data: any) => {
        ...
        chart = new Chart(ctx, RunsChartOptions(this.inningsInfo[0].batting.totals, this.inningsInfo[0].team))

        ctx = this.runsChart2.nativeElement.getContext('2d');
        chart = new Chart(ctx, RunsChartOptions(this.inningsInfo[1].batting.totals, this.inningsInfo[1].team))
      })
